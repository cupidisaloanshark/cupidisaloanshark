

<div align="center">

```
╔══════════════════════════════════════════════════════════╗
║           ★  CHARACTER STATUS WINDOW  ★                  ║
║                    LVL 7 DEVELOPER                       ║
╚══════════════════════════════════════════════════════════╝
```

</div>

---

<table width="100%">
<tr>
<td width="35%" valign="top">

```
┌─────────────────────┐
│                     │
│         🧙          │
│                     │
│  ┌───────────────┐  │
│  │  HERO_DEV     │  │
│  │ IA Researcher │  │
│  │    Warrior    │  │
│  └───────────────┘  │
└─────────────────────┘
```

**▸ VITAIS**

```
HP  [████████░░]  87/100
MP  [█████████░]  45/50
☕  [█████████░]  CHEIO
```

**▸ COMBAT STATS**

| Atributo | Valor |
|----------|-------|
| STR (commits) | 347 |
| DEX (PR speed) | 89% |
| INT (stars ⭐) | 142 |
| WIS (repos) | 38 |
| LCK (bugs) | baixo |

</td>
<td width="65%" valign="top">

**▸ HABILIDADES**

```
JavaScript  [█████████░]  90 / 100
TypeScript  [███████░░░]  78 / 100
React       [████████░░]  85 / 100
Node.js     [███████░░░]  72 / 100
Python      [██████░░░░]  60 / 100
SQL         [██████░░░░]  65 / 100
```

---

**▸ EQUIPAMENTO EQUIPADO**

[![My Skills](https://skillicons.dev/icons?i=js,ts,react,nodejs,python,postgres,docker,git)](https://skillicons.dev)

---

**▸ MISSÕES ATIVAS**

```
◈ [EM PROGRESSO] Construir API RESTful com autenticação JWT
◎ [ACEITANDO   ] Contribuir para projeto open source
◇ [PLANEJANDO  ] Aprender Rust (linguagem lendária)
```

---

**▸ XP ANUAL**

```
2026 [███████░░░░░░░░] 3600 / 5000 XP  ▮
```

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=SEU_USUARIO&show_icons=true&theme=github_dark&hide_border=true&title_color=c9a227&icon_color=c9a227&text_color=e6edf3&bg_color=161b22)

</td>
</tr>
</table>

---

<div align="center">

**▸ CONQUISTAS DESBLOQUEADAS**

![GitHub Streak](https://streak-stats.demolab.com?user=SEU_USUARIO&theme=dark&hide_border=true&background=161B22&ring=C9A227&fire=C9A227&currStreakLabel=C9A227)

</div>

---

<div align="center">

**▸ TOP LINGUAGENS**

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USUARIO&layout=compact&theme=github_dark&hide_border=true&title_color=c9a227&text_color=e6edf3&bg_color=161b22)

</div>

---

<div align="center">

**▸ MAPA DE ATIVIDADE (SNAKE EDITION)**

<!-- Habilite a GitHub Action para gerar essa snake dinamicamente! -->
<!-- Veja: https://github.com/Platane/snk -->

![Snake Animation](https://github.com/SEU_USUARIO/SEU_USUARIO/blob/output/github-contribution-grid-snake-dark.svg)

</div>

---

<div align="center">

```
[Z] Ver Projetos    [X] Entrar em Contato    [C] Ver Blog
```

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/SEU_PERFIL)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:SEU@EMAIL.COM)
[![Portfolio](https://img.shields.io/badge/Portfolio-C9A227?style=for-the-badge&logo=firefox&logoColor=white)](https://SEU_SITE.com)

```
★ OPEN TO WORK ★   Visitantes: ![Visitor Count](https://profile-counter.glitch.me/SEU_USUARIO/count.svg)
```

</div>
